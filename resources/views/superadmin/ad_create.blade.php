<form method="POST" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('superadmin.ad_create') }}">
	@csrf
    <div class="input-wrap mt-2">
        <label for="title" class="eForm-label">{{ get_phrase('Title') }}</label>
        <input type="text" class="form-control eForm-control" name="title" id="title" placeholder="{{ get_phrase('Ad title') }}" required>
    </div>
    ...
    <div class="input-wrap mt-2">
        ...
        <select class="form-select eForm-select eChoice-multiple-without-remove" name="product_id" id="product_id">
            @foreach ($products as $key => $product)
                <option value="{{ $product->id }}">{{ get_phrase($product->name) }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="input-wrap mt-2">
        ...
        <select class="form-select eForm-select eChoice-multiple-without-remove" name="ad_dimension_id" id="ad_dimension_id">
            @foreach ($ad_dimensions as $key => $ad_dimension)
                <option value="{{ $ad_dimension->id }}">{{ get_phrase($ad_dimension->dimension) }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="input-wrap mt-2">
        ...
        <input type="text" class="form-control eForm-control" name="link_to_click" id="link_to_click" placeholder="{{ get_phrase('Ad link') }}" required>
    </div>
    ...
    <div class="input-wrap mt-2">
        ...
        <div class="ciRadio-wrap">
            <div class="form-check">
                <input class="form-check-input ciRadio ciRadio-OutlinePrimary" type="radio" name="ad_type" id="ad_type" value="text"/>
                <label class="form-check-label" for="flexRadioPrimaryOutline">{{ get_phrase('Only Text') }}</label>
            </div>
            <div class="form-check">
                <input class="form-check-input ciRadio ciRadio-OutlinePrimary" type="radio" name="ad_type" id="ad_type" value="image"/>
    ...
    <div class="input-wrap mt-2">
        ...
        <div class="input-group">
            <textarea class="form-control eForm-control" name="short_description" id="short_description" rows="4" spellcheck="false"></textarea>
        </div>
    </div>
