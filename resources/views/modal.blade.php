...
<div class="modal eModal fade" id="confirmSweetAlerts" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered sweet-alerts text-sweet-alerts">
        <div class="modal-content">
            <div class="modal-body">
                <div class="icon icon-confirm mb-3">
                    <svg xmlns="http://www.w3.org/2000/svg" height="48" width="48">
                        <path d="M22.5 29V10H25.5V29ZM22.5 38V35H25.5V38Z" fill="rgb(255 85 119)" />
                    </svg>
                </div>
                <p class="text-center">{{ get_phrase('Are you sure?') }}</p>
                <p class="focus-text text-center">You won't able to revert this!</p>
                <div class="confirmBtn mt-3 justify-content-center d-flex gap-3">
                    <a href="javascript:;" id="confirmBtn" class="">
                        <button type="button" id="confirmBtn" class="btn btn-danger">{{ get_phrase('Yes') }}</button>
                    </a>
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">{{ get_phrase('Cancel') }}</button>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function elementModal(url) {
        jQuery('#element-modal').modal('show', {
            backdrop: 'true'
        });
        // SHOW AJAX RESPONSE ON REQUEST SUCCESS
...
<script type="text/javascript">
    ...
    function elementCheckoutModal(url) {
        ...
        // LOADING THE AJAX MODAL
        jQuery('#elementCheckoutModal').modal('show', {
            backdrop: 'true'
        });
